...
Um fato importante a respeito dos dados dos discursos é que os mesmos na verdade são uma transcrição do que foi falado em plenário. Nesta transcrição algumas infomações textuais foram inseridas  que representam situações ocorridas na sessão. Por exemplo, em alguns discursos existem  referências a palmas como (.Palmas). Em outros existema inserções de pequenos trecho de fala do presidente da sessão, agradecendo pelo discurso. Algumas destas  ocorrências podem ser removidas facilmente, mas  outras não poderão e serão consideradas como parte do discurso.
...
Isto de fato é um problemas pois em algumas situações o nomes em questão não estão iguais nas duas fontes. Logo, teremos que realizar verificação deste casos, executando um tratamento diferenciado que em último consiste em  uma intervenção  manual.

Que tipo de transformação precisará ser realizada?
---------------------

Pretendemos utilizar técnicas de Mineração de Text (Text Mining)  com suas fase de pré processamento: tokenização, conversão para minúsculo, exclusão de pontuação, exclusão de números e de palavras de paradas (stop words).
...
Nesta fase da pesquisa não visualisamos nenhuma atividade de transformação com objetivo de corrigir problema de obliquidade e sobreposição. Mas tal correção não está descartada.
