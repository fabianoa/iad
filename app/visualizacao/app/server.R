...
dadosDeputados<-readRDS(paste(getwd(),"/data/quantitativoDeputadosPartidos.Rda",sep = ''))
...
shinyServer(function(input, output, session) {
    ...
    # Fill in the spot we created for a plot
    output$composicaoPartidariaDashboard <- renderPlot({
    ...
    output$US.ActivityMeasures.Dashboard <- renderGvis({
        ...
        Data        <- data.frame(Period = index(Data), Data)
        Data        <- Data[Data$Period>"2007-12-01",]
        names(Data) <- c("Period", 
    ...
    misc.plot.EnsembleForecasting <- function(EnsembleForecast, ChartTitle=""){
        plot(EnsembleForecast[,1], col="black", lwd=2, 
             ...
             sub="Note: Green=Actuals; Red = Mean Forecast; Intervall Ranges: 10%-90%; 25-75%; 40-60%")
    ...
    EnsembleForecast.calc <- reactive({
        if (Data.Change) {
            ...
            Table.ForecastTracking.Update <- data.frame(ForecastDate = Sys.Date(),
                                                        NewRelease = NA,
                                                        ValueNewRelease = NA,
                                                        ForecastVariable = NA, #input$ForecastPooling.Selection,
            ...
            if (input$ForecastPooling.Selection == "GDP")              {
                ...
                dropbox_save(dropbox_credentials, Prior.ForecastResult.GDP, file='work/bac/AutomatedForecastingWithShiny/PriorGDPForecast.rda') 
            }
            ...
            # dropbox_save(dropbox_credentials, Prior.ForecastResult, file='work/bac/AutomatedForecastingWithShiny/PriorForecast.rda')
            cat("\n    - Saved as prior forecast in Dropbox")
        } else {
            if (input$ForecastPooling.Selection == "GDP")              Forecast.Result <- Prior.ForecastResult.GDP
            if (input$ForecastPooling.Selection == "Nonfarm Payrolls") Forecast.Result <- Prior.ForecastResult.NFP
            cat("\n     --> Using forecast results stored previously")
        }
        return(Forecast.Result)
    })
    
    output$Forecast.Tracking <- renderText({
        if (Data.Change)  return(print(Table.Change)) else return(NULL)
    })
